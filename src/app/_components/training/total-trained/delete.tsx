...
export const DeleteCumulativeTraining = ({ id }: { id: string }) => {
  const queryClient = useQueryClient();
  const deleteCumulative = useMutation({
    mutationKey: ['delete-cumulative-training'],
    mutationFn: async () => {
      return await api
        .delete(`/training/cumulative/${id}`)
        .then((res) => {
          if (!res.data.success) {
    ...
    onSuccess: (data) => {
      ...
      toast.success('Total Training Data deleted successfully');
    },
  });
  return (
    <Button
