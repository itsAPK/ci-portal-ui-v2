...
export const AutoComplete = ({
  ...
  return (
    <CommandPrimitive onKeyDown={handleKeyDown} className="bg-white  rounded  ">
      <CommandInput
        ref={inputRef}
        onValueChange={(e) => {
          // if (!isLoading) {
        ...
        disabled={disabled}
        className="h-9 w-[500px] bg-white text-[15px] opacity-100"
      />

      <CommandList className='bg-white opacity-100'>
        {isLoading ? (
        ...
        {options.length > 0 && !isLoading ? (
          <CommandGroup className="bg-white opacity-100" onSelect={(e) => e.preventDefault()}>
            {options.map((option) => {
              ...
              return (
                <CommandItem
                  ...
                  className={cn('flex w-full bg-white gap-2', !isSelected ? 'pl-8' : null)}
                >
